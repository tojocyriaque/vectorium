...
def figure_out_value_at_time(sorted_kfs, t, prop, default=0):
    """
...
def clean_up_messy_ai_json(raw: str) -> dict:
    """
...
def enforce_physics_rules(data: dict) -> dict:
    """
    ...
    for obj in data.get("objects", []):
        ...
        if vectors is not None:
            ...
            for v in vectors:
                ...
                v_x = figure_out_value_at_time(fixed, v_t, "x", 300)
                v_y = figure_out_value_at_time(fixed, v_t, "y", 200)
                fixed_vecs.append({
...
def ask_groq_for_animation(description: str) -> dict:
    ...
    return clean_up_messy_ai_json(res.json()["choices"][0]["message"]["content"])


def ask_gemini_for_animation(description: str) -> dict:
    ...
    res.raise_for_status()
    return clean_up_messy_ai_json(res.json()["candidates"][0]["content"]["parts"][0]["text"])
